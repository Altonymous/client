...
func (arg LoadUserArg) WithDoServerMerklePolling() LoadUserArg {
	arg.doMerkleServerPolling = true
	return arg
}
...
func LoadUser(arg LoadUserArg) (ret *User, err error) {
	...
	ret, refresh, refreshReason, err = loadUser(m, arg.uid, resolveBody, sigHints, arg.forceReload, arg.merkleLeaf, MerkleOpts{DoServerPolling: arg.doMerkleServerPolling})
	if err != nil {
		return nil, err
	}
...
func loadUser(m MetaContext, uid keybase1.UID, resolveBody *jsonw.Wrapper, sigHints *SigHints, force bool, leaf *MerkleUserLeaf, mlo MerkleOpts) (*User, bool, string, error) {
	...
	if leaf == nil {
		leaf, err = lookupMerkleLeaf(m, uid, (local != nil), sigHints, mlo)
		if err != nil {
...
func lookupMerkleLeaf(m MetaContext, uid keybase1.UID, localExists bool, sigHints *SigHints, mlo MerkleOpts) (f *MerkleUserLeaf, err error) {
	if uid.IsNil() {
	...
	f, err = m.G().MerkleClient.LookupUser(m, q, sigHints, mlo)
...
func lookupSigHintsAndMerkleLeaf(m MetaContext, uid keybase1.UID, localExists bool, mlo MerkleOpts) (sigHints *SigHints, leaf *MerkleUserLeaf, err error) {
	...
	leaf, err = lookupMerkleLeaf(m, uid, true, sigHints, mlo)
	if err != nil {
		return nil, nil, err
	}
